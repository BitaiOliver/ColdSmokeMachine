...
void setup() {
  ...
  timerISR.attach(0.1,HandleTimerInterrupt);
...
void HandleTimerInterrupt(void)
{
  ...
  if (0U == (GlobalTimer % TIMER_100MS))
  {
    /* Flag 1s Task as to be executed */
    Rte_IWrite_TaskMgr_Handler_Flags |= 1 << TIMER_100MS_ID;
  }
  else
  {
    /* do nothing */
  }
  if (0U == (GlobalTimer % TIMER_200MS))
  {
    /* Flag 1s Task as to be executed */
    Rte_IWrite_TaskMgr_Handler_Flags |= 1 << TIMER_200MS_ID;
  }
  else
  {
    /* do nothing */
